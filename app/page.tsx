...
import { Eye, EyeOff, Mail, Lock, Store, Bus, ShoppingCart, Info, ArrowRight, Smartphone, ShieldAlert, Key, ChevronRight } from "lucide-react";
...
export default function Home() {
  ...
  const [step, setStep] = useState(1); // 1: Login Form, 2: Choose 2FA Method, 3: Enter Verification Code
  const [mfaMethod, setMfaMethod] = useState<"otp" | "authenticator">("otp");
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTempToken(data.session.accessToken);
      setStep(2);
    } catch (err: any) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    localStorage.setItem("paynexToken", tempToken);
      ...
      <div className="flex flex-col w-full lg:w-1/2 p-8 md:p-12 relative overflow-hidden bg-gradient-to-br from-blue-50/80 via-white to-white">
        ...
        <div className="max-w-[420px] w-full mx-auto flex flex-col justify-center flex-1">
          {step === 1 ? (
            ...
            <>
              <h1 className="text-[28px] font-bold mb-2 tracking-tight text-gray-900">Two-step verification</h1>
              <p className="text-gray-500 mb-8 text-sm">
                Select your preferred second verification step to secure your account.
              </p>
              ...
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="w-full p-4 bg-white border border-gray-200 hover:border-blue-600 hover:bg-blue-50/10 rounded-2xl flex items-center justify-between transition-all text-left shadow-sm cursor-pointer"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-gray-900 text-sm">Send code to OTP</p>
                      <p className="text-xs text-gray-500 font-medium">Get a 6-digit code sent to your email</p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-gray-900 text-sm">Via Authenticator app</p>
                      <p className="text-xs text-gray-500 font-medium">Use code from Google Authenticator or Duo</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-gray-400" />
                </button>
              ...
              <div className="text-center mt-8">
                <button
                  type="button"
                  onClick={() => setStep(1)}
                  className="text-sm text-blue-600 hover:underline font-medium bg-transparent border-none cursor-pointer p-0"
                >
                  Back to Login
                </button>
              </div>
            ...
            <>
              <h1 className="text-[32px] font-bold mb-2 tracking-tight text-gray-900">
                {mfaMethod === "otp" ? "Two-step verification" : "Authenticator App"}
              </h1>
              <p className="text-gray-500 mb-8 text-sm">
                {mfaMethod === "otp" ? (
                  <>
                    We've sent a 6-digit verification code to <span className="font-semibold text-gray-800">{email}</span>. Enter the code below.
                  </>
                ) : (
                  "Open your Authenticator app (e.g. Google Authenticator) and enter the 6-digit verification code below."
                )}
              </p>
                ...
                <div className="flex gap-2 justify-between">
                  {otp.map((digit, idx) => (
                    <input
                      ...
                      className="w-[50px] h-12 text-center text-xl font-bold border border-gray-200 rounded-xl focus:border-blue-600 focus:ring-1 focus:ring-blue-600 bg-white outline-none transition-all"
                      required
                    />
                  ))}
                </div>
              ...
              <div className="flex flex-col gap-3 items-center mt-6">
                {mfaMethod === "otp" && (
                  <button
                    ...
                    className="text-xs text-gray-500 hover:text-gray-700 transition-colors font-medium bg-transparent border-none cursor-pointer p-0"
                  >
